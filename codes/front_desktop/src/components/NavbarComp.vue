<template>
  <nav
    :class="`relative text-branco-normal h-screen p-5 pt-8 duration-300 card-shadow rounded-r-[16px] ${
      isOpen ? 'w-60' : 'w-20'
    }`"
  >
    <i
      ...
      text-3xl rounded-full absolute -right-4 top-9 cursor-pointer
    ...
    <div
      :class="`relative -bottom-40 w-full h-auto flex items-center mb-4 rounded-lg ${
        this.isOpen ? '' : 'justify-center'
      }`"
    >
      <li
        class="
...
export default {
  data() {
    return {
      isOpen: true,
      logo: [
        require("@/assets/logo-icon-gradient.png"),
        require("@/assets/logo-inline-gradient.png"),
      ],
    };
  },
  methods: {
