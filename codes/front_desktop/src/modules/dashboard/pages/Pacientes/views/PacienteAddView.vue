<template>
  ...
  <main class="h-screen w-full p-8 overflow-auto">
    ...
    <span class="block mb-5 font-montserrat pl-2 text-lg text-cinza-4">
      <router-link to="/pacientes" class="text-cinza-4">Pacientes</router-link>
      /
      <span class="text-cinza-1"> Adicionar Paciente </span>
    </span>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <div v-if="Number(user.isAdmin)" class="mb-8">
            <span class="font-montserrat text-xl block mb-4"
...
export default {
  ...
  methods: {
    ...mapActions("pacientes", ["ActionPostPacient", "setModalPacient"]),
    ...
    async onSubmit(values) {
      ...
      formData.append("user_id", `${user_id ? user_id : this.user.id}`);

      await this.ActionPostPacient(formData)
        .then((res) => {
          if (res == 200) {
            this.setModalPacient(true);

            setTimeout(() => {
              this.setModalPacient(false);
