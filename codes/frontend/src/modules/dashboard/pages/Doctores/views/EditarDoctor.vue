...
import { editar } from "@/utils/doctor";
...
export default {
  ...
  setup() {
    ...
    async function onSubmit(data) {
      const response = await DoctorStore.EditarDoctor(data, state.imagem);
    ...
    onMounted(() => DoctorStore.SelecionarDoctor(route.params.id));

    state.doctor = computed(() => DoctorStore.doctorSelecionado);

    return { schema, onSubmit, fecharMenu, previewImage, state };
  },
};
</script>
...
<template>
  <DefaultLayout>
    <div class="container mx-auto mb-20">
      <Header page="Editar Informações do Doctor" />
              ...
              <div class="w-full flex justify-center mb-4">
                <img
                  ...
                  :src="`${
                    state.preview
                      ? state.preview
                      : `http://localhost:8000/storage/${state.doctor.imagem}`
                  }`"
                  alt="Foto de perfil"
                />
              </div>
              ...
              <div class="mb-4 flex gap-4">
                <div class="form-add">
                  ...
                  <Field
                    name="especialidade"
                    as="select"
                    v-model="state.doctor.especialidade"
                    class="text-sm appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                  >
                ...
                <div class="form-add">
                  ...
                  <Field
                    type="text"
                    name="idCarteira"
                    v-model="state.doctor.idCarteira"
                    class="text-sm appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
